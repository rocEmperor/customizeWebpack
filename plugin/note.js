 ...
 * Webpack 的事件流机制应用了观察者模式，和 Node中的 EventEmitter 非常相似，可以直接在 Compiler Compilation 对象上广播和监听事件，方法如下：
 * 事件触发：compiler.apply ('event-name', params); 【event-name 为事件名称，params 为附带的参数】
 * 事件注册：compiler.plugin('event-name', function(params) {})
 * Compiler和Compilation(它们是 Plugin和Webpack之间的桥梁。 Compiler和Compilation 的含义如下。)
